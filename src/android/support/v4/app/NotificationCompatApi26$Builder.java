    ...
    implements NotificationBuilderWithBuilderAccessor, NotificationBuilderWithActions
{
    ...
    private android.app._cls26.Builder.mB mB;

    public void addAction(cessor cessor)
    {
        NotificationCompatApi24.addAction(mB, cessor);
    }
    ...
    public android.app._cls26.Builder getBuilder()
    {
        return mB;
    }

    cessor(Context context, Notification notification, CharSequence charsequence, CharSequence charsequence1, CharSequence charsequence2, RemoteViews remoteviews, int i, 
    ...
    {
        context = (new android.app._cls26.Builder(context)).Builder(notification.when).(flag1).n(notification.icon, notification.iconLevel).n(notification.contentView).n(notification.tickerText, remoteviews).n(notification.sound, notification.audioStreamType).n(notification.vibrate).n(notification.ledARGB, notification.ledOnMS, notification.ledOffMS);
        ...
        mB = context.enIntent(pendingintent1, flag1).n(bitmap).n(i).nometer(flag2).(l).(j, k, flag).y(flag3).y(bundle).y(s1).mary(flag4).mary(s2).(s).(i1).ty(j1).rsion(notification1).putHistory(acharsequence).putHistory(s3);
        if (remoteviews1 != null)
        {
            mB.ntentView(remoteviews1);
        }
        if (remoteviews2 != null)
        {
            mB.gContentView(remoteviews2);
        }
        if (remoteviews3 != null)
        {
            mB.adsUpContentView(remoteviews3);
        }
        for (context = arraylist.iterator(); context.hasNext(); mB.mB(notification))
        {
